...
import { Table, Button, Modal, Form, Input, Select, InputNumber, Space, message, Typography, Tag, Popconfirm } from 'antd';
...
const RuleScoreConfig: React.FC = () => {
  ...
  const columns = [
    ...
    { title: '数据类型', dataIndex: 'data_type', render: (t: string) => <Tag color={DATA_TYPE_COLORS[t]}>{t}</Tag> },
    {
      title: '约束', render: (_: any, r: any) => {
        ...
        if (r.data_type === 'CATEGORICAL') return (r.categories || []).map((c: any) => c.label).join(', ');
        return '-';
      },
    },
    {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        <Typography.Title level={5} style={{ margin: 0 }}>评分配置模板</Typography.Title>
        <Button type="primary" size="small" icon={<PlusOutlined />} onClick={() => openEdit()}>新建配置</Button>
      </div>

      <Table dataSource={data || []} rowKey="id" loading={isLoading} columns={columns} pagination={false} size="small" />
...
export default RuleScoreConfig;
